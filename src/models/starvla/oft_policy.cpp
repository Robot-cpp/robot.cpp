...
class OFTGGUFLoader final : public gguf_loader {
  ...
  protected:
    bool parse_metadata(gguf_context * gguf) override {
        ...
        if (require_i32(gguf, "starvla.schema_version") != 1 || require_string(gguf, "starvla.framework") != "oft") {
        ...
        if (config_.backbone_arch != "qwen3_vl" && config_.backbone_arch != "qwen2_5_vl") {
            throw std::runtime_error("StarVLA OFT policy has an unsupported Qwen backbone");
        }
        ...
        if (config_.input_dim <= 0 || config_.input_embedding_dim <= 0 || config_.vocab_size <= 0 ||
            config_.hidden_dim <= 0 || config_.block_count <= 0 || config_.action_dim <= 0 || config_.horizon <= 0 ||
            !std::isfinite(config_.layer_norm_epsilon) || config_.layer_norm_epsilon <= 0.0f) {
        ...
        if (!validate_oft_prompt_config(config_.prompt, prompt_error) || config_.action_token_id < 0) {
            throw std::runtime_error(prompt_error.empty() ? "StarVLA OFT token/template metadata is incompatible"
                                                          : prompt_error);
        }
        ...
        if (config_.image_count <= 0 || config_.image_names.size() != static_cast<size_t>(config_.image_count) ||
...
std::unique_ptr<OFTPolicy> OFTPolicy::load(const std::string & path, int n_threads, int verbosity,
    ...
    try {
        ...
        impl->build_graph();
        if (verbosity >= 1) {
            std::fprintf(stderr, "%s: backend=%s input=%d hidden=%d blocks=%d horizon=%d action_dim=%d profiles=%zu\n",
...
bool OFTPolicy::evaluate(const float * action_queries, size_t element_count, std::vector<float> & normalized_actions,
                         std::string & error) {
    ...
    const size_t expected = static_cast<size_t>(impl_->config.horizon) * static_cast<size_t>(impl_->config.input_dim);
    if (action_queries == nullptr || element_count != expected) {
        error = "StarVLA OFT action-query tensor has an incompatible shape";
        return false;
    }

    if (impl_->scheduler == nullptr || impl_->graph == nullptr || impl_->input == nullptr || impl_->output == nullptr) {
    ...
    const size_t output_count =
        static_cast<size_t>(impl_->config.horizon) * static_cast<size_t>(impl_->config.action_dim);
