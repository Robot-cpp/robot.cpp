...
int require_key(gguf_context * gguf, const char * key, gguf_type type) {
    ...
    if (index < 0) {
        throw std::runtime_error(std::string("missing required StarVLA PI GGUF metadata: ") +
                                 key);
    }
    if (gguf_get_kv_type(gguf, index) != type) {
        throw std::runtime_error(std::string("invalid StarVLA PI GGUF metadata type: ") +
                                 key);
    }
    return index;
}
...
int require_array(gguf_context * gguf, const char * key, gguf_type element_type) {
    ...
    if (gguf_get_arr_type(gguf, index) != element_type) {
        throw std::runtime_error(
            std::string("invalid StarVLA PI GGUF array element type: ") + key);
    }
    return index;
}
...
std::vector<int32_t> require_i32_array(gguf_context * gguf, const char * key) {
    ...
    if (data == nullptr && count != 0) {
        throw std::runtime_error(std::string("missing StarVLA PI GGUF array data: ") +
                                 key);
    }
    return count == 0 ? std::vector<int32_t>()
                      : std::vector<int32_t>(data, data + count);
}
...
std::string profile_key(int profile_index, const char * suffix) {
    return "starvla.normalization.profile." + std::to_string(profile_index) + "." +
           suffix;
}

bool has_shape(const ggml_tensor * tensor,
               std::initializer_list<int64_t> expected) {
    if (tensor == nullptr ||
        static_cast<size_t>(ggml_n_dims(tensor)) != expected.size()) {
        return false;
    }
    size_t dimension = 0;
  ...
  protected:
    bool parse_metadata(gguf_context * gguf) override {
        ...
        normalization.clip_actions =
            require_bool(gguf, "starvla.normalization.clip_actions");
        ...
        if (profile_count <= 0 ||
            keys.size() != static_cast<size_t>(profile_count)) {
            throw std::runtime_error(
                "StarVLA PI normalization profile count is inconsistent");
        }
        ...
        for (int i = 0; i < profile_count; ++i) {
            ...
            profile.key =
                require_string(gguf, profile_key(i, "key").c_str());
            profile.action_q01 =
                require_f32_array(gguf, profile_key(i, "action_q01").c_str());
            profile.action_q99 =
                require_f32_array(gguf, profile_key(i, "action_q99").c_str());
            profile.action_mask =
                require_bool_array(gguf,
                                   profile_key(i, "action_mask").c_str());
            if (profile.key != keys[static_cast<size_t>(i)]) {
                throw std::runtime_error(
                    "StarVLA PI normalization profile order is inconsistent");
            }
