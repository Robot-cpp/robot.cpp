...
std::vector<int32_t> expected_hidden_tuple_indices(int qwen_layer_count, int block_count) {
...
class PIGGUFLoader final : public gguf_loader {
  public:
    PIGGUFLoader(PIPolicyConfig & config, PIWeights & weights) : config_(config), weights_(weights) {}

  protected:
    bool parse_metadata(gguf_context * gguf) override {
        if (require_string(gguf, "general.architecture") != "starvla-policy" ||
            require_i32(gguf, "starvla.schema_version") != 1 || require_string(gguf, "starvla.framework") != "pi") {
        ...
        if (config_.backbone_arch != "qwen2_5_vl" || config_.bundle_uuid.empty() || config_.text_filename.empty() ||
        ...
        config_.qwen_hidden_tuple_indices = require_i32_array(gguf, "starvla.conditioning.hidden_tuple_indices");
        ...
        config_.attention_head_count    = require_i32(gguf, "starvla.pi.attention_head_count");
        config_.attention_head_dim      = require_i32(gguf, "starvla.pi.attention_head_dim");
        config_.cross_attention_dim     = require_i32(gguf, "starvla.pi.cross_attention_dim");
        config_.feed_forward_dim        = require_i32(gguf, "starvla.pi.feed_forward_dim");
        config_.mlp_hidden_dim          = require_i32(gguf, "starvla.pi.mlp_hidden_dimension");
        ...
        config_.state_token_count       = require_i32(gguf, "starvla.pi.state_token_count");
        config_.future_token_count      = require_i32(gguf, "starvla.pi.future_token_count");
        config_.action_position_count   = require_i32(gguf, "starvla.pi.action_position_count");
        config_.timestep_projection_dim = require_i32(gguf, "starvla.pi.timestep_projection_dim");
        config_.num_inference_timesteps = require_i32(gguf, "starvla.pi.num_inference_timesteps");
        config_.ada_norm_epsilon        = require_f32(gguf, "starvla.pi.ada_norm_epsilon");
        config_.euler_dt                = require_f32(gguf, "starvla.pi.euler_dt");
        config_.timestep_ids            = require_i32_array(gguf, "starvla.pi.timestep_ids");

        const std::vector<int32_t> expected_indices =
            expected_hidden_tuple_indices(config_.qwen_layer_count, config_.block_count);
        const bool dimensions_valid =
            config_.qwen_hidden_dim > 0 && config_.qwen_input_embedding_dim == config_.qwen_hidden_dim &&
            config_.qwen_layer_count >= config_.block_count && config_.qwen_vocab_size > 0 && config_.dit_width > 0 &&
            config_.dit_width % 2 == 0 && config_.block_count > 0 && config_.attention_head_count > 0 &&
            ...
            config_.mlp_hidden_dim > 0 && config_.state_dim > 0 && config_.action_dim > 0 && config_.horizon > 0 &&
            config_.state_token_count == 1 && config_.future_token_count > 0 &&
            config_.action_position_count >= config_.horizon && config_.timestep_projection_dim >= 4 &&
            config_.timestep_projection_dim % 2 == 0 && config_.num_inference_timesteps > 0 &&
            config_.timestep_ids.size() == static_cast<size_t>(config_.num_inference_timesteps) &&
            std::isfinite(config_.ada_norm_epsilon) && config_.ada_norm_epsilon > 0.0f &&
            std::isfinite(config_.euler_dt) && config_.euler_dt > 0.0f &&
            config_.qwen_hidden_tuple_indices == expected_indices && config_.image_count > 0 &&
            config_.image_names.size() == static_cast<size_t>(config_.image_count) &&
            config_.image_framework_inference_pre_resize_width > 0 &&
            config_.image_framework_inference_pre_resize_height > 0 && config_.image_processor_min_pixels > 0 &&
            ...
            config_.image_max_token_count >= config_.image_min_token_count && !config_.cot_template.empty();
        if (!dimensions_valid) {
            throw std::runtime_error("StarVLA PI dimensions, hidden taps, or sampler schedule are incompatible");
        }
        ...
        if (!normalization.clip_actions || normalization.binary_comparison != "ge") {
            throw std::runtime_error("StarVLA PI normalization must clip actions and use "
    ...
    bool bind_tensors(ggml_context * ctx_data) override {
        ...
        bind(weights_.timestep_input_weight, "starvla.policy.pi.timestep.input.weight");
        bind(weights_.timestep_input_bias, "starvla.policy.pi.timestep.input.bias");
        bind(weights_.timestep_output_weight, "starvla.policy.pi.timestep.output.weight");
        bind(weights_.timestep_output_bias, "starvla.policy.pi.timestep.output.bias");
        ...
        for (int block = 0; block < config_.block_count; ++block) {
            const std::string prefix = "starvla.policy.pi.block." + std::to_string(block) + ".";
        ...
        bind(weights_.state_input_weight, "starvla.policy.pi.state.input.weight");
        bind(weights_.state_input_bias, "starvla.policy.pi.state.input.bias");
        bind(weights_.state_output_weight, "starvla.policy.pi.state.output.weight");
        bind(weights_.state_output_bias, "starvla.policy.pi.state.output.bias");
        bind(weights_.action_input_weight, "starvla.policy.pi.action.input.weight");
        bind(weights_.action_input_bias, "starvla.policy.pi.action.input.bias");
        bind(weights_.action_time_mix_weight, "starvla.policy.pi.action.time_mix.weight");
        bind(weights_.action_time_mix_bias, "starvla.policy.pi.action.time_mix.bias");
        bind(weights_.action_output_weight, "starvla.policy.pi.action.output.weight");
        bind(weights_.action_output_bias, "starvla.policy.pi.action.output.bias");
        bind(weights_.velocity_input_weight, "starvla.policy.pi.velocity.input.weight");
        bind(weights_.velocity_input_bias, "starvla.policy.pi.velocity.input.bias");
        bind(weights_.velocity_output_weight, "starvla.policy.pi.velocity.output.weight");
        bind(weights_.velocity_output_bias, "starvla.policy.pi.velocity.output.bias");
        bind(weights_.future_tokens, "starvla.policy.pi.future_tokens.weight");
        bind(weights_.action_position, "starvla.policy.pi.action_position.weight");
        ...
        if (!has_shape(weights_.timestep_input_weight, {config_.timestep_projection_dim, width}) ||
            ...
            !has_shape(weights_.state_output_weight, {config_.mlp_hidden_dim, width}) ||
            ...
            throw std::runtime_error("StarVLA PI non-transformer tensor has an incompatible ggml shape");
        }
        for (const PIBlockWeights & block : weights_.blocks) {
            if (!has_shape(block.ada_norm_weight, {width, 2 * width}) || !has_shape(block.ada_norm_bias, {2 * width}) ||
                !has_shape(block.query_weight, {width, width}) || !has_shape(block.query_bias, {width}) ||
                !has_shape(block.key_weight, {config_.cross_attention_dim, width}) ||
                !has_shape(block.key_bias, {width}) ||
                !has_shape(block.value_weight, {config_.cross_attention_dim, width}) ||
                !has_shape(block.value_bias, {width}) || !has_shape(block.attention_output_weight, {width, width}) ||
                !has_shape(block.attention_output_bias, {width}) ||
                !has_shape(block.feed_forward_input_weight, {width, config_.feed_forward_dim}) ||
                !has_shape(block.feed_forward_input_bias, {config_.feed_forward_dim}) ||
                !has_shape(block.feed_forward_output_weight, {config_.feed_forward_dim, width}) ||
                !has_shape(block.feed_forward_output_bias, {width})) {
                throw std::runtime_error("StarVLA PI transformer tensor has an incompatible ggml shape");
            }
        }
        return true;
    }
...
std::vector<float> timestep_projection_table(const PIPolicyConfig & config) {
    std::vector<float> result(static_cast<size_t>(config.num_inference_timesteps) * config.timestep_projection_dim);
    ...
    for (int step = 0; step < config.num_inference_timesteps; ++step) {
        ...
        for (int i = 0; i < half; ++i) {
            const float exponent                           = -std::log(10000.0f) * i / static_cast<float>(half - 1);
            const float angle                              = timestep * std::exp(exponent);
            const size_t offset                            = static_cast<size_t>(step) * config.timestep_projection_dim;
...
std::vector<float> action_time_table(const PIPolicyConfig & config) {
    std::vector<float> result(static_cast<size_t>(config.num_inference_timesteps) * config.dit_width);
    const int half = config.dit_width / 2;
    for (int step = 0; step < config.num_inference_timesteps; ++step) {
        const float timestep = static_cast<float>(config.timestep_ids[static_cast<size_t>(step)]);
        for (int i = 0; i < half; ++i) {
            const float exponent                           = -std::log(10000.0f) * i / static_cast<float>(half);
            const float angle                              = timestep * std::exp(exponent);
            const size_t offset                            = static_cast<size_t>(step) * config.dit_width;
...
struct PIPolicy::Impl {
    ...
    void build_graph(size_t token_count, bool include_state) {
        ...
        if (token_count == 0 || token_count > static_cast<size_t>(std::numeric_limits<int>::max())) {
            throw std::runtime_error("invalid StarVLA PI conditioning token count");
        }
        ...
        if (graph_context == nullptr) {
            throw std::runtime_error("failed to initialize StarVLA PI graph context");
        }
        ...
        if (include_state) {
            state_input = ggml_new_tensor_1d(graph_context, GGML_TYPE_F32, config.state_dim);
        }
        noise_input               = ggml_new_tensor_2d(graph_context, GGML_TYPE_F32, config.action_dim, config.horizon);
        timestep_projection_input = ggml_new_tensor_2d(graph_context, GGML_TYPE_F32, config.timestep_projection_dim,
                                                       config.num_inference_timesteps);
        action_time_input = ggml_new_tensor_2d(graph_context, GGML_TYPE_F32, width, config.num_inference_timesteps);
        scalar_one_input  = ggml_new_tensor_1d(graph_context, GGML_TYPE_F32, 1);
        if (hidden_input == nullptr || (include_state && state_input == nullptr) || noise_input == nullptr ||
            timestep_projection_input == nullptr || action_time_input == nullptr || scalar_one_input == nullptr) {
            throw std::runtime_error("failed to create StarVLA PI graph inputs");
        }
        ...
        ggml_set_name(timestep_projection_input, "starvla_pi_timestep_projection_table");
        ...
        auto ada_norm = [&](ggml_tensor * value, ggml_tensor * temb, const PIBlockWeights & block) {
            ...
            return ggml_add(graph_context,
                            ggml_mul(graph_context, normalized, ggml_add(graph_context, scale, scalar_one_input)),
                            shift);
        };
        auto attention = [&](ggml_tensor * query_source, ggml_tensor * key_value_source, const PIBlockWeights & block) {
            ...
            scores =
                ggml_soft_max_ext(graph_context, scores, nullptr, 1.0f / std::sqrt(static_cast<float>(head_dim)), 0.0f);
        ...
        if (include_state) {
            state_features =
                ggml_relu(graph_context, linear(state_input, weights.state_input_weight, weights.state_input_bias));
            state_features = linear(state_features, weights.state_output_weight, weights.state_output_bias);
            state_features = ggml_reshape_2d(graph_context, state_features, width, 1);
        }
        ...
        for (int step = 0; step < config.num_inference_timesteps; ++step) {
            ...
            if (state_features != nullptr) {
                hidden = ggml_concat(graph_context, state_features, hidden, 1);
            }
            ...
            for (int block_index = 0; block_index < config.block_count; ++block_index) {
                const PIBlockWeights & block = weights.blocks[static_cast<size_t>(block_index)];
                ggml_tensor * layer_hidden =
                    ggml_view_2d(graph_context, hidden_input, config.qwen_hidden_dim, static_cast<int64_t>(token_count),
                                 hidden_input->nb[1], static_cast<size_t>(block_index) * hidden_input->nb[2]);
                ggml_tensor * normalized = ada_norm(hidden, temb, block);
                hidden                   = ggml_add(graph_context, hidden, attention(normalized, layer_hidden, block));
            ...
            ggml_tensor * velocity = ggml_view_2d(
                ...
                static_cast<size_t>((include_state ? config.state_token_count : 0) + config.future_token_count) *
                    hidden->nb[1]);
        ...
        if (graph == nullptr) {
            throw std::runtime_error("failed to create StarVLA PI graph");
        }
        ggml_build_forward_expand(graph, output);
        ggml_backend_sched_reset(scheduler);
        if (!ggml_backend_sched_alloc_graph(scheduler, graph)) {
            throw std::runtime_error("failed to allocate StarVLA PI graph");
        }
...
std::unique_ptr<PIPolicy> PIPolicy::load(const std::string & path, int n_threads, int verbosity, std::string & error) {
    error.clear();
    if (path.empty()) {
...
bool PIPolicy::evaluate(const float * qwen_hidden_states, size_t hidden_element_count, const float * state,
                        size_t state_element_count, const float * initial_noise, size_t noise_element_count,
    ...
    const size_t layer_width = static_cast<size_t>(impl_->config.block_count) * impl_->config.qwen_hidden_dim;
    if (qwen_hidden_states == nullptr || layer_width == 0 || hidden_element_count == 0 ||
        hidden_element_count % layer_width != 0) {
        error = "StarVLA PI layer-wise Qwen conditioning tensor has an incompatible shape";
        return false;
    }
    ...
    if (include_state && (state == nullptr || state_element_count != static_cast<size_t>(impl_->config.state_dim))) {
    ...
    if (std::any_of(qwen_hidden_states, qwen_hidden_states + hidden_element_count,
        ...
        (include_state &&
         std::any_of(state, state + state_element_count, [](float value) { return !std::isfinite(value); })) ||
        std::any_of(initial_noise, initial_noise + noise_element_count,
                    [](float value) { return !std::isfinite(value); })) {
        error = "StarVLA PI conditioning, state, and initial noise must be finite";
        return false;
    }

    try {
        if (impl_->graph == nullptr || impl_->conditioning_token_count != token_count ||
    ...
    if (include_state) {
        ggml_backend_tensor_set(impl_->state_input, state, 0, state_element_count * sizeof(float));
    }
...
bool PIPolicy::unnormalize(const std::vector<float> & normalized_actions, const std::string & profile_key_value,
