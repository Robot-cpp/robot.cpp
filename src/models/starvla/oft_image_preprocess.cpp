...
bool validate_resize(const uint8_t * source, int source_width, int source_height,
                     int source_stride, int target_width, int target_height,
                     std::string & error) {
    error.clear();
    if (source == nullptr || source_width <= 0 || source_height <= 0 || target_width <= 0 ||
        target_height <= 0) {
    ...
    const uint64_t output_bytes = static_cast<uint64_t>(target_width) *
...
FilterTable make_filter_table(int input_size, int output_size, bool pillow_precision) {
    ...
    std::vector<double> floating_weights(static_cast<size_t>(output_size) *
                                         static_cast<size_t>(table.kernel_size), 0.0);
    ...
    for (int output = 0; output < output_size; ++output) {
        ...
        for (int index = 0; index < count; ++index) {
            const double distance =
                (static_cast<double>(index + first) - center + 0.5) / filter_scale;
        ...
        if (sum != 0.0) {
            for (int index = 0; index < count; ++index) {
                double & weight =
                    floating_weights[static_cast<size_t>(output) * table.kernel_size + index];
                weight /= sum;
    ...
    if (pillow_precision) {
        ...
        for (table.precision = 0; table.precision < 22; ++table.precision) {
            const int next = static_cast<int>(
                0.5 + maximum_weight * static_cast<double>(uint32_t{1} << (table.precision + 1)));
            if (next >= (1 << 15)) {
                break;
            }
        }
...
RGBImage resize_horizontal(const RGBImage & source, int target_width,
            ...
            for (int channel = 0; channel < 3; ++channel) {
                ...
                for (int index = 0; index < count; ++index) {
                    const size_t source_index =
                        (static_cast<size_t>(row) * source.width + first + index) * 3 + channel;
                    const int32_t weight =
                        table.weights[static_cast<size_t>(column) * table.kernel_size + index];
                    accumulator += static_cast<int64_t>(source.pixels[source_index]) * weight;
                }
                const size_t target_index =
                    (static_cast<size_t>(row) * target.width + column) * 3 + channel;
...
RGBImage resize_vertical(const RGBImage & source, int target_height,
                         const FilterTable & table) {
    RGBImage target;
        ...
        for (int column = 0; column < target.width; ++column) {
            for (int channel = 0; channel < 3; ++channel) {
                ...
                for (int index = 0; index < count; ++index) {
                    ...
                    const int32_t weight =
                        table.weights[static_cast<size_t>(row) * table.kernel_size + index];
                    accumulator += static_cast<int64_t>(source.pixels[source_index]) * weight;
                }
                const size_t target_index =
                    (static_cast<size_t>(row) * target.width + column) * 3 + channel;
...
bool resize_rgb(const uint8_t * source, int source_width, int source_height, int source_stride,
                int target_width, int target_height, bool pillow_precision,
                std::vector<uint8_t> & target, std::string & error) {
    target.clear();
    if (!validate_resize(source, source_width, source_height, source_stride, target_width,
                         target_height, error)) {
        return false;
    }
    ...
    if (source_width != target_width) {
        current = resize_horizontal(current, target_width,
                                    make_filter_table(source_width, target_width, pillow_precision));
    }
    if (source_height != target_height) {
        current = resize_vertical(current, target_height,
                                  make_filter_table(source_height, target_height, pillow_precision));
    }
...
bool resize_pillow_bicubic_rgb(const uint8_t * source, int source_width, int source_height,
                               int source_stride, int target_width, int target_height,
                               std::vector<uint8_t> & target, std::string & error) {
    return resize_rgb(source, source_width, source_height, source_stride, target_width,
                      target_height, true, target, error);
}
...
bool resize_torchvision_bicubic_aa_rgb(const uint8_t * source, int source_width,
                                       ...
                                       std::string & error) {
    return resize_rgb(source, source_width, source_height, source_stride, target_width,
                      target_height, false, target, error);
}
...
bool qwen3vl_smart_resize_dimensions(int source_width, int source_height, int factor,
    ...
    target_height = 0;
    error.clear();
    if (source_width <= 0 || source_height <= 0 || factor <= 0 || min_pixels <= 0 ||
    ...
    if (rounded_pixels > max_pixels) {
        ...
        resized_height = std::max<int64_t>(
            factor, static_cast<int64_t>(std::floor(source_height / beta / factor)) * factor);
        resized_width = std::max<int64_t>(
            factor, static_cast<int64_t>(std::floor(source_width / beta / factor)) * factor);
    } else if (rounded_pixels < min_pixels) {
        const double beta = std::sqrt(static_cast<double>(min_pixels) / source_pixels);
        resized_height =
            static_cast<int64_t>(std::ceil(source_height * beta / factor)) * factor;
        resized_width =
            static_cast<int64_t>(std::ceil(source_width * beta / factor)) * factor;
    }

    if (resized_width <= 0 || resized_height <= 0 ||
        resized_width > std::numeric_limits<int>::max() ||
...
bool preprocess_qwen3vl_rgb(const uint8_t * source, int source_width, int source_height,
    ...
    if (!qwen3vl_smart_resize_dimensions(source_width, source_height, factor, min_pixels,
                                         max_pixels, target_width, target_height, error)) {
        return false;
    }
    if (!resize_torchvision_bicubic_aa_rgb(source, source_width, source_height, source_stride,
                                           target_width, target_height, target, error)) {
        target_width = 0;
        target_height = 0;
        return false;
    }
...
bool preprocess_oft_rgb(const uint8_t * source, int source_width, int source_height,
    ...
    if (!resize_pillow_bicubic_rgb(source, source_width, source_height, source_stride,
                                   training_width, training_height, training_image, error)) {
        return false;
    }
    return resize_torchvision_bicubic_aa_rgb(
        training_image.data(), training_width, training_height, training_width * 3,
