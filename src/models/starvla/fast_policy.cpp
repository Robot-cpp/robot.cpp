...
FastRuntimeMetadata parse_metadata(gguf_context * gguf, FastPolicyConfig & config) {
    ...
    runtime.codec.vocab_size     = static_cast<size_t>(require_i32(gguf, "starvla.fast.codec.vocab_size"));
    runtime.codec.time_horizon   = static_cast<size_t>(require_i32(gguf, "starvla.fast.codec.time_horizon"));
    runtime.codec.action_dim     = static_cast<size_t>(require_i32(gguf, "starvla.fast.codec.action_dimension"));
    ...
    const bool valid =
        ...
        config.image_count > 0 && config.image_names.size() == static_cast<size_t>(config.image_count) &&
        ...
        runtime.codec.vocab_size > 0 && action_token_count == static_cast<int>(runtime.codec.vocab_size) &&
...
RawTensor read_tensor(const std::string & path, gguf_context * gguf, ggml_context * metadata_context, const char * name,
    ...
    if (!stream || offset > static_cast<size_t>(std::numeric_limits<std::streamoff>::max())) {
...
std::unique_ptr<FastPolicy> FastPolicy::load(const std::string & path, int verbosity, std::string & error) {
    error.clear();
    if (path.empty()) {
    ...
    try {
        ...
        RawTensor action_map = read_tensor(path, gguf, metadata_context, kActionMapTensor, GGML_TYPE_I32,
                                           static_cast<int64_t>(runtime.codec.vocab_size));
...
bool FastPolicy::decode_generated(const std::vector<int32_t> & full_sequence, std::vector<float> & normalized_actions,
    ...
    if (decoded.batch_size != 1 || decoded.time_horizon != static_cast<size_t>(impl_->config.horizon) ||
        decoded.action_dim != static_cast<size_t>(impl_->config.action_dim) ||
        decoded.actions.size() != static_cast<size_t>(impl_->config.horizon * impl_->config.action_dim)) {
...
bool FastPolicy::unnormalize(const std::vector<float> & normalized_actions, const std::string & profile_key,
