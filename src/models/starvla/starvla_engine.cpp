...
const char * starvla_variant_name(StarVLAVariant variant) noexcept {
    ...
    case StarVLAVariant::qwen3_groot:
        return "qwen3_groot";
    case StarVLAVariant::qwen3_pi_v3:
        return "qwen3_pi_v3";
    case StarVLAVariant::qwen25_oft:
        return "qwen25_oft";
    case StarVLAVariant::qwen25_groot:
        return "qwen25_groot";
    case StarVLAVariant::qwen25_pi:
        return "qwen25_pi";
    case StarVLAVariant::qwen25_fast:
        return "qwen25_fast";
    }
    return "unknown";
}
...
bool starvla_variant_from_metadata(const std::string & framework, const std::string & backbone,
                                   StarVLAVariant & variant) noexcept {
    if (backbone == "qwen3_vl") {
        if (framework == "oft")
            variant = StarVLAVariant::qwen3_oft;
        else if (framework == "groot")
            variant = StarVLAVariant::qwen3_groot;
        else if (framework == "pi_v3")
            variant = StarVLAVariant::qwen3_pi_v3;
        else
            return false;
        return true;
    }
    if (backbone == "qwen2_5_vl") {
        if (framework == "oft")
            variant = StarVLAVariant::qwen25_oft;
        else if (framework == "groot")
            variant = StarVLAVariant::qwen25_groot;
        else if (framework == "pi")
            variant = StarVLAVariant::qwen25_pi;
        else if (framework == "fast")
            variant = StarVLAVariant::qwen25_fast;
        else
            return false;
        return true;
    }
...
bool read_policy_variant(const std::filesystem::path & path, StarVLAVariant & variant, std::string & error) {
    ...
    std::string backbone;
    const bool valid = read_string("starvla.framework", framework) && read_string("starvla.backbone.arch", backbone);
    gguf_free(gguf);
    if (!valid) {
        return false;
    }
...
bool is_plain_basename(const std::string & value) {
    ...
    return !path.has_root_path() && !path.has_parent_path() && path.filename() == path && value != "." && value != "..";
}

bool require_regular_file(const std::filesystem::path & path, const char * label, std::string & error) {
...
bool resolve_component_path(const std::string & metadata_filename, const std::string & component_path,
                            const char * label, std::filesystem::path & resolved, std::string & error) {
    if (!is_plain_basename(metadata_filename)) {
        error = std::string("StarVLA policy ") + label + " filename must be a plain basename: " + metadata_filename;
        return false;
    }

    if (component_path.empty() || component_path.find('\0') != std::string::npos) {
        error = std::string("StarVLA ") + label + " path is required and must not contain an embedded NUL";
        return false;
    }
    resolved = std::filesystem::path(component_path);
    if (resolved.filename().string() != metadata_filename) {
        error = std::string("StarVLA ") + label + " basename must match policy metadata '" + metadata_filename +
                "': " + resolved.string();
        return false;
    }
...
bool validate_observation(const observation & obs, int image_count, const std::vector<std::string> & image_names,
    ...
    if (obs.images.size() != static_cast<size_t>(image_count)) {
        error = label + " requires exactly " + std::to_string(image_count) + " image(s) in policy order";
        return false;
    }
    if (obs.task.empty()) {
    ...
    for (size_t i = 0; i < obs.images.size(); ++i) {
        ...
        if (image.name != image_names[i]) {
            error = label + " image " + std::to_string(i) + " must be named '" + image_names[i] + "'";
            return false;
        }
        ...
        if (image.stride_bytes < 0 || (image.stride_bytes != 0 && image.stride_bytes < packed_stride)) {
            error = label + " image '" + image.name + "' stride is smaller than a packed RGB row";
            return false;
        }
    }
    return true;
...
bool prepare_qwen_images(const observation & obs, const PolicyConfig & config, const char * framework,
    ...
    for (size_t i = 0; i < obs.images.size(); ++i) {
        ...
        if (!preprocess_qwen3vl_rgb(
                image.data, image.width, image.height, image.channels, image.stride_bytes, config.image_patch_size,
                config.image_spatial_merge_size, config.image_processor_min_pixels, config.image_processor_max_pixels,
                processed_images[i], target_width, target_height, image_token_count, preprocess_error)) {
            error = std::string("failed to preprocess StarVLA ") + framework + " image '" + image.name +
                    "': " + preprocess_error;
            return false;
        }
        const uint64_t expected_bytes = static_cast<uint64_t>(target_width) * static_cast<uint64_t>(target_height) * 3;
        if (expected_bytes != processed_images[i].size() || image_token_count < config.image_min_token_count ||
            image_token_count > config.image_max_token_count) {
            error = std::string("StarVLA ") + framework + " image preprocessor returned an incompatible dynamic grid";
            return false;
        }
...
bool prepare_pi_qwen_images(const observation & obs, const PIPolicyConfig & config,
    ...
    for (size_t i = 0; i < obs.images.size(); ++i) {
        const model_image & image = obs.images[i];
        std::string preprocess_error;
        if (!resize_torchvision_bicubic_aa_rgb(image.data, image.width, image.height, image.stride_bytes,
            ...
            error = "failed to pre-resize StarVLA PI image '" + image.name + "': " + preprocess_error;
            return false;
        }
        ...
        int target_height     = 0;
        int image_token_count = 0;
        if (!preprocess_qwen3vl_rgb(pre_resized_images[i].data(), config.image_framework_inference_pre_resize_width,
                                    config.image_framework_inference_pre_resize_height, 3,
                                    config.image_framework_inference_pre_resize_width * 3, config.image_patch_size,
                                    config.image_spatial_merge_size, config.image_processor_min_pixels,
                                    config.image_processor_max_pixels, processed_images[i], target_width, target_height,
                                    image_token_count, preprocess_error)) {
            error = "failed to preprocess StarVLA PI image '" + image.name + "': " + preprocess_error;
            return false;
        }
        const uint64_t expected_bytes = static_cast<uint64_t>(target_width) * static_cast<uint64_t>(target_height) * 3;
        if (expected_bytes != processed_images[i].size() || image_token_count < config.image_min_token_count ||
            image_token_count > config.image_max_token_count) {
            error = "StarVLA PI image preprocessor returned an incompatible dynamic grid";
            return false;
        }
        Qwen3VLImageView view;
        view.data         = processed_images[i].data();
...
std::unique_ptr<StarVLAEngine> StarVLAEngine::load(const StarVLAEngineConfig & config, std::string & error) {
    error.clear();
    ...
    const bool is_oft   = impl->variant == StarVLAVariant::qwen3_oft || impl->variant == StarVLAVariant::qwen25_oft;
    const bool is_groot = impl->variant == StarVLAVariant::qwen3_groot || impl->variant == StarVLAVariant::qwen25_groot;
    ...
    const NormalizationConfig * normalization = nullptr;
    if (is_oft) {
        impl->oft_policy = OFTPolicy::load(impl->policy_path.string(), effective_threads, config.verbosity, error);
        ...
        impl->groot_policy = GR00TPolicy::load(impl->policy_path.string(), effective_threads, config.verbosity, error);
        ...
        impl->pi_policy = PIPolicy::load(impl->policy_path.string(), effective_threads, config.verbosity, error);
        ...
        impl->pi_v3_policy = PIV3Policy::load(impl->policy_path.string(), effective_threads, config.verbosity, error);
        ...
        impl->fast_policy = FastPolicy::load(impl->policy_path.string(), config.verbosity, error);
        ...
        image_spatial_merge_size        = policy.image_spatial_merge_size;
        normalization                   = &policy.normalization;
        if (config.n_ctx < static_cast<int>(policy.generation_max_length)) {
            error = "StarVLA FAST --n-ctx must be at least max_length=" + std::to_string(policy.generation_max_length);
            return nullptr;
        }
    }
    if (!resolve_component_path(text_filename, config.text_path, "text GGUF", impl->text_path, error) ||
        !resolve_component_path(mmproj_filename, config.mmproj_path, "mmproj GGUF", impl->mmproj_path, error)) {
        return nullptr;
    }
    if (same_file(impl->policy_path, impl->text_path) || same_file(impl->policy_path, impl->mmproj_path) ||
    ...
    const NormalizationProfile * profile = resolve_normalization_profile(*normalization, "", profile_error);
    if (profile == nullptr) {
        error = std::string("failed to select StarVLA ") + framework + " normalization profile: " + profile_error;
        return nullptr;
    }
    ...
    const QwenVLArchitecture expected_architecture =
        qwen_backbone_arch == "qwen2_5_vl" ? QwenVLArchitecture::qwen2_5_vl : QwenVLArchitecture::qwen3_vl;
    ...
    if (config.verbosity >= 1) {
        ...
        const char * policy_backend = is_oft ? impl->oft_policy->backend_name()
                                             : (is_groot ? impl->groot_policy->backend_name()
                                                         : (is_pi ? impl->pi_policy->backend_name()
                                                                  : (is_pi_v3 ? impl->pi_v3_policy->backend_name()
                                                                              : impl->fast_policy->backend_name())));
        std::fprintf(stderr,
                     "%s: variant=%s policy=%s text=%s mmproj=%s profile=%s "
                     "qwen_backend=%s policy_backend=%s\n",
                     __func__, variant_name, impl->policy_path.string().c_str(), impl->text_path.string().c_str(),
                     impl->mmproj_path.string().c_str(), impl->normalization_profile_key.c_str(),
                     impl->qwen->backend_name(), policy_backend);
    }
    return std::unique_ptr<StarVLAEngine>(new StarVLAEngine(std::move(impl)));
}

bool StarVLAEngine::predict(const observation & obs, StarVLAEngineResult & result, std::string & error) {
    ...
    if (impl_ == nullptr || impl_->qwen == nullptr ||
        (!impl_->oft_policy && !impl_->groot_policy && !impl_->pi_policy && !impl_->pi_v3_policy &&
         !impl_->fast_policy)) {
    ...
    const NormalizationProfile * profile =
        resolve_normalization_profile(*impl_->normalization, impl_->normalization_profile_key, profile_error);
    if (profile == nullptr) {
    ...
    const auto make_noise = [&](size_t count, std::vector<float> & noise) {
        if (!obs.initial_noise.empty()) {
            if (obs.initial_noise.size() != count || !std::all_of(obs.initial_noise.begin(), obs.initial_noise.end(),
        ...
        for (float & value : noise) {
            value = ggml_bf16_to_fp32(ggml_fp32_to_bf16(normal(impl_->noise_rng)));
        }
        return true;
    };
    ...
    if (impl_->variant == StarVLAVariant::qwen25_fast) {
        ...
        if (!validate_observation(obs, config.image_count, config.image_names, false, "FAST", error)) {
            return fail();
        }
        ...
        if (!prepare_qwen_images(obs, config, "FAST", processed_images, qwen_images, error)) {
            return fail();
        }
        ...
        generation.repetition_penalty = config.generation_repetition_penalty;
        QwenVLGenerationResult generated;
        stage_start = Clock::now();
        if (!impl_->qwen->generate_autoregressive(qwen_images, instruction, generation, generated, error)) {
            error = "StarVLA FAST Qwen2.5-VL generation failed: " + error;
            return fail();
        }
        result.timings.qwen3vl_ms = elapsed_ms(stage_start, Clock::now());
        if (generated.prompt_token_count == 0 || generated.full_sequence.size() < generated.prompt_token_count ||
            generated.full_sequence.size() > config.generation_max_length) {
            error = "StarVLA FAST Qwen2.5-VL returned an incompatible generated sequence";
            return fail();
        }
        stage_start = Clock::now();
        if (!impl_->fast_policy->decode_generated(generated.full_sequence, normalized_actions, error)) {
        ...
        stage_start = Clock::now();
        if (!impl_->fast_policy->unnormalize(normalized_actions, profile->key, result.actions, error)) {
        ...
        if (result.actions.size() != expected_actions || normalized_actions.size() != expected_actions ||
            !std::all_of(result.actions.begin(), result.actions.end(),
                         [](float action) { return std::isfinite(action); })) {
            error = "StarVLA FAST returned an incompatible or non-finite action tensor";
            return fail();
        }
    ...
    if (impl_->variant == StarVLAVariant::qwen25_pi) {
        ...
        if (!validate_observation(obs, config.image_count, config.image_names, true, "PI", error)) {
            return fail();
        }
        if (!obs.state.empty() && obs.state.size() != static_cast<size_t>(config.state_dim)) {
            error =
                "StarVLA PI accepts either no state or exactly " + std::to_string(config.state_dim) + " state values";
            return fail();
        }
        ...
        std::vector<std::vector<uint8_t>> processed_images;
        std::vector<Qwen3VLImageView> qwen_images;
        if (!prepare_pi_qwen_images(obs, config, pre_resized_images, processed_images, qwen_images, error)) {
            return fail();
        }
        result.timings.image_preprocess_ms = elapsed_ms(stage_start, Clock::now());
        ...
        stage_start = Clock::now();
        if (!impl_->qwen->extract_layer_hidden_states(qwen_images, instruction, config.qwen_hidden_tuple_indices,
                                                      hidden_states, attention_mask, error)) {
            error = "StarVLA PI Qwen2.5-VL inference failed: " + error;
            return fail();
        }
        result.timings.qwen3vl_ms    = elapsed_ms(stage_start, Clock::now());
        const size_t expected_hidden = static_cast<size_t>(config.block_count) * attention_mask.size() *
                                       static_cast<size_t>(config.qwen_hidden_dim);
        if (attention_mask.empty() || hidden_states.size() != expected_hidden) {
            error = "StarVLA PI Qwen2.5-VL returned an incompatible layerwise conditioning shape";
            return fail();
        }

        std::vector<float> noise;
        if (!make_noise(static_cast<size_t>(config.horizon) * config.action_dim, noise)) {
            return fail();
        }

        stage_start = Clock::now();
        if (!impl_->pi_policy->evaluate(hidden_states.data(), hidden_states.size(),
                                        obs.state.empty() ? nullptr : obs.state.data(), obs.state.size(), noise.data(),
                                        noise.size(), normalized_actions, error)) {
        ...
        stage_start = Clock::now();
        if (!impl_->pi_policy->unnormalize(normalized_actions, profile->key, result.actions, error)) {
        ...
        if (result.actions.size() != expected_actions ||
            !std::all_of(result.actions.begin(), result.actions.end(),
                         [](float action) { return std::isfinite(action); })) {
            error = "StarVLA PI returned an incompatible or non-finite action tensor";
            return fail();
        }
        result.chunk_size       = config.horizon;
        result.action_dim       = config.action_dim;
    ...
    if (impl_->variant == StarVLAVariant::qwen3_pi_v3) {
        ...
        if (!validate_observation(obs, config.image_count, config.image_names, false, "PI_v3", error)) {
            return fail();
        }
        ...
        const size_t expected_hidden =
            static_cast<size_t>(config.qwen_layer_count) * attention_mask.size() * config.qwen_hidden_dim;
        if (attention_mask.empty() || hidden_states.size() != expected_hidden) {
        ...
        stage_start = Clock::now();
        if (!impl_->pi_v3_policy->evaluate(hidden_states.data(), hidden_states.size(), attention_mask.data(),
        ...
        stage_start = Clock::now();
        if (!impl_->pi_v3_policy->unnormalize(normalized_actions, profile->key, result.actions, error)) {
    ...
    if (impl_->variant == StarVLAVariant::qwen3_groot || impl_->variant == StarVLAVariant::qwen25_groot) {
        const GR00TPolicyConfig & config = impl_->groot_policy->config();
        if (!validate_observation(obs, config.image_count, config.image_names, false, "GR00T", error)) {
            return fail();
        }
        ...
        std::vector<uint8_t> attention_mask;
        stage_start = Clock::now();
        if (!impl_->qwen->extract_full_hidden_states(qwen_images, instruction, hidden_states, attention_mask, error)) {
            error = "StarVLA GR00T Qwen3-VL inference failed: " + error;
            return fail();
        }
        result.timings.qwen3vl_ms = elapsed_ms(stage_start, Clock::now());
        if (hidden_states.empty() || hidden_states.size() % static_cast<size_t>(config.qwen_hidden_dim) != 0 ||
            hidden_states.size() / static_cast<size_t>(config.qwen_hidden_dim) != attention_mask.size()) {
        ...
        stage_start = Clock::now();
        if (!impl_->groot_policy->evaluate(hidden_states.data(), hidden_states.size(), attention_mask.data(),
                                           attention_mask.size(), noise.data(), noise.size(), normalized_actions,
                                           error)) {
        ...
        stage_start = Clock::now();
        if (!impl_->groot_policy->unnormalize(normalized_actions, profile->key, result.actions, error)) {
    ...
    stage_start = Clock::now();
    if (!impl_->qwen->extract_token_embeddings(qwen_images, instruction, config.action_token_id,
    ...
    stage_start = Clock::now();
    if (!impl_->oft_policy->evaluate(action_queries.data(), action_queries.size(), normalized_actions, error)) {
    ...
    stage_start = Clock::now();
    if (!impl_->oft_policy->unnormalize(normalized_actions, profile->key, result.actions, error)) {
