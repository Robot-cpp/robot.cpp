  ...
  protected:
    bool parse_metadata(gguf_context * gguf) override {
        ...
        if (require_i32(gguf, "starvla.schema_version") != 1 ||
            require_string(gguf, "starvla.framework") != "groot") {
        ...
        if (config_.backbone_arch != "qwen3_vl" &&
            ...
            throw std::runtime_error(
                "StarVLA GR00T policy has an unsupported Qwen backbone");
        }
        ...
        const int64_t expected_input_embedding_dim =
            qwen25 ? static_cast<int64_t>(config_.qwen_hidden_dim)
                   : 4LL * config_.qwen_hidden_dim;
        const bool dimensions_valid =
            config_.qwen_hidden_dim > 0 && config_.qwen_vocab_size > 0 && config_.dit_width > 0 &&
            config_.qwen_input_embedding_dim == expected_input_embedding_dim &&
            config_.dit_width % 2 == 0 &&
            ...
            config_.output_dim > 0 && config_.mlp_hidden_dim > 0 &&
            config_.action_dim > 0 && config_.horizon > 0 && config_.future_token_count > 0 &&
        ...
        if (profile_count <= 0 || keys.size() != static_cast<size_t>(profile_count)) {
            throw std::runtime_error("StarVLA GR00T normalization profile count is inconsistent");
        }
        normalization.profiles.clear();
        for (int profile_index = 0; profile_index < profile_count; ++profile_index) {
            NormalizationProfile profile;
            profile.key = require_string(gguf, profile_key(profile_index, "key").c_str());
            profile.action_q01 =
                require_f32_array(gguf, profile_key(profile_index, "action_q01").c_str());
            profile.action_q99 =
                require_f32_array(gguf, profile_key(profile_index, "action_q99").c_str());
            profile.action_mask =
                require_bool_array(gguf, profile_key(profile_index, "action_mask").c_str());
            if (profile.key != keys[static_cast<size_t>(profile_index)]) {
                throw std::runtime_error("StarVLA GR00T normalization profile order is inconsistent");
            }
    ...
    bool bind_tensors(ggml_context * ctx_data) override {
        ...
        for (int block = 0; block < config_.block_count; ++block) {
            ...
            if (!has_shape(current.ada_norm_weight, {width, 2 * width}) ||
                ...
                !has_shape(current.key_weight, {kv_input_dim, width}) ||
                !has_shape(current.key_bias, {width}) ||
                !has_shape(current.value_weight, {kv_input_dim, width}) ||
...
struct GR00TPolicy::Impl {
    ...
    void build_graph(size_t token_count) {
        ...
        hidden_input = ggml_new_tensor_2d(graph_context, GGML_TYPE_F32, config.qwen_hidden_dim,
        ...
        auto ada_norm = [&](ggml_tensor * value, ggml_tensor * temb, const GR00TBlockWeights & block) {
            ggml_tensor * modulation = linear(ggml_silu(graph_context, temb), block.ada_norm_weight,
                                              block.ada_norm_bias);
        ...
        auto attention = [&](ggml_tensor * query_source, ggml_tensor * key_value_source,
                             ggml_tensor * mask, const GR00TBlockWeights & block) {
            ...
            scores = ggml_soft_max_ext(graph_context, scores, mask,
        ...
        for (int step = 0; step < 4; ++step) {
            ...
            ggml_tensor * output_modulation = linear(ggml_silu(graph_context, temb),
                                                      weights.output_modulation_weight,
            ...
            ggml_tensor * scale = ggml_view_1d(graph_context, output_modulation, width,
                                               static_cast<size_t>(width) * sizeof(float));
            ...
            hidden = ggml_relu(graph_context,
                               linear(hidden, weights.velocity_input_weight, weights.velocity_input_bias));
...
std::unique_ptr<GR00TPolicy> GR00TPolicy::load(const std::string & path, int n_threads, int verbosity,
    ...
    try {
        ...
        if (verbosity >= 1) {
            std::fprintf(stderr,
                         "%s: backend=%s qwen=%d width=%d blocks=%d horizon=%d action_dim=%d profiles=%zu\n",
                         __func__, mode_name(impl->mode), impl->config.qwen_hidden_dim, impl->config.dit_width,
...
bool GR00TPolicy::evaluate(const float * qwen_hidden_states, size_t hidden_element_count,
    ...
    if (qwen_hidden_states == nullptr || qwen_attention_mask == nullptr || initial_noise == nullptr ||
        ...
        mask_element_count > std::numeric_limits<size_t>::max() /
                                 static_cast<size_t>(impl_->config.qwen_hidden_dim) ||
    ...
    for (int query = 0; query < query_count; ++query) {
        ...
        for (size_t token = 0; token < mask_element_count; ++token) {
            row[token] = qwen_attention_mask[token] != 0 ? 0.0f :
                                                           -std::numeric_limits<float>::infinity();
        }
    }
...
bool GR00TPolicy::unnormalize(const std::vector<float> & normalized_actions,
                              const std::string & profile_key_value, std::vector<float> & actions,
