...
class PIPolicy {
  public:
    ...
    static std::unique_ptr<PIPolicy> load(const std::string & path, int n_threads, int verbosity, std::string & error);
    ...
    // [horizon, action_dim].
    bool evaluate(const float * qwen_hidden_states, size_t hidden_element_count, const float * state,
