...
StarVLAModel::StarVLAModel(std::unique_ptr<starvla::StarVLAEngine> engine) : engine_(std::move(engine)) {}
...
bool make_starvla_model(const model_args & args, std::unique_ptr<Model> & out, std::string & error) {
    ...
    if (!is_starvla_model_type(args.type)) {
        error = std::string("model type '") + model_type_name(args.type) + "' is not a StarVLA model type";
        return false;
    }
    ...
    std::unique_ptr<starvla::StarVLAEngine> engine = starvla::StarVLAEngine::load(config, error);
    if (engine == nullptr) {
        return false;
    }

    std::unique_ptr<StarVLAModel> model(new (std::nothrow) StarVLAModel(std::move(engine)));
    if (model == nullptr) {
