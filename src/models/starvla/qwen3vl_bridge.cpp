...
bool qwen_vl_resolve_architecture(const std::string & text_architecture, const std::string & projector_type,
    ...
    error.clear();
    if (text_architecture == "qwen2vl" && projector_type == "qwen2.5vl_merger") {
        architecture = QwenVLArchitecture::qwen2_5_vl;
        return true;
    }
    if (text_architecture == "qwen3vl" && projector_type == "qwen3vl_merger") {
        architecture = QwenVLArchitecture::qwen3_vl;
        return true;
    }
    if (text_architecture != "qwen2vl" && text_architecture != "qwen3vl") {
        error = "unsupported Qwen-VL text architecture: " + text_architecture;
    } else if (projector_type != "qwen2.5vl_merger" && projector_type != "qwen3vl_merger") {
...
bool qwen_vl_is_final_norm_tensor_name(const char * name) noexcept {
    return name != nullptr && (std::strcmp(name, "result_norm") == 0 || std::strcmp(name, "result_embd_pooled") == 0);
}
...
bool qwen_vl_hidden_state_source(QwenVLArchitecture architecture, int decoder_layer_count, int deepstack_layer_count,
    ...
    if (architecture == QwenVLArchitecture::qwen3_vl) {
        if (deepstack_layer_count <= 0 || deepstack_layer_count > decoder_layer_count) {
            error = "Qwen3-VL DeepStack layer count is incompatible with the model";
            return false;
        }
        source.kind  = hidden_tuple_index <= deepstack_layer_count ? QwenVLHiddenStateSourceKind::deepstack_output
...
bool qwen_vl_select_repetition_penalized_top1(const float * logits, size_t vocab_size,
    ...
    token = -1;
    error.clear();
    if (logits == nullptr || vocab_size == 0 || vocab_size > static_cast<size_t>(INT32_MAX) ||
...
bool finish_layer_capture(LayerCapture & capture) {
    ...
    return qwen_bf16_capture_download_cuda(capture.cuda_capture, capture.values.data(), capture.values.size(),
                                           capture.error);
#endif
    return true;
}
...
void begin_layer_boundary_tracking(LayerCapture & capture, size_t layer_count) {
    ...
    capture.rounded_deepstack_layers.assign(capture.expected_deepstack_layer_count, uint8_t{0});
}
...
bool validate_layer_boundary_tracking(const LayerCapture & capture, std::string & error) {
    ...
    if (capture.rounded_layers.size() != capture.layer_to_slot.size() ||
        capture.rounded_deepstack_layers.size() != capture.expected_deepstack_layer_count ||
        std::any_of(capture.rounded_layers.begin(), capture.rounded_layers.end(),
                    [](uint8_t seen) { return seen != 1; }) ||
        std::any_of(capture.rounded_deepstack_layers.begin(), capture.rounded_deepstack_layers.end(),
                    [](uint8_t seen) { return seen != 1; })) {
        error = "Qwen3-VL BF16 residual-boundary roundtrip did not observe "
...
bool observe_backend_placement(ggml_tensor * tensor, bool ask, void * user_data) {
    if (!ask || tensor == nullptr || tensor->op == GGML_OP_NONE || tensor->buffer == nullptr || user_data == nullptr) {
        return false;
    }
    ...
    ggml_backend_dev_t device = buffer_type == nullptr ? nullptr : ggml_backend_buft_get_device(buffer_type);
    if (device == nullptr) {
        return false;
    }
...
bool observe_text_and_capture_layers(ggml_tensor * tensor, bool ask, void * user_data) {
    ...
    const bool is_result_norm = qwen_vl_is_final_norm_tensor_name(tensor->name);
    const bool valid_deepstack_layer =
        deepstack_layer >= 0 && static_cast<size_t>(deepstack_layer) < capture->deepstack_to_slot.size();
    const bool valid_layer = layer >= 0 && static_cast<size_t>(layer) < capture->layer_to_slot.size();
    ...
    const bool round_layer     = capture->bf16_residual_layer_boundaries && valid_layer;
    const bool round_deepstack = capture->bf16_residual_layer_boundaries && valid_deepstack_layer &&
                                 static_cast<size_t>(deepstack_layer) < capture->expected_deepstack_layer_count;
    ...
    try {
        ...
        if (round_layer && (static_cast<size_t>(layer) >= capture->rounded_layers.size() ||
                            capture->rounded_layers[static_cast<size_t>(layer)] != 0)) {
            capture->error = "Qwen3-VL l_out BF16 roundtrip index is invalid or repeated";
            return false;
        }
        if (round_deepstack && (static_cast<size_t>(deepstack_layer) >= capture->rounded_deepstack_layers.size() ||
                                capture->rounded_deepstack_layers[static_cast<size_t>(deepstack_layer)] != 0)) {
            capture->error = "Qwen3-VL deepstack_out BF16 roundtrip index is invalid or repeated";
            return false;
        }
        if (!ggml_is_contiguous(tensor) || tensor->ne[0] != static_cast<int64_t>(capture->hidden_size) ||
            tensor->ne[1] != static_cast<int64_t>(capture->token_count) || tensor->ne[2] != 1 || tensor->ne[3] != 1) {
            capture->error = "Qwen3-VL hidden-state output has an incompatible shape or layout";
            return false;
        }
        if (capture->hidden_size == 0 ||
            capture->token_count > std::numeric_limits<size_t>::max() / capture->hidden_size) {
        ...
        if (count > std::numeric_limits<size_t>::max() / sizeof(float) ||
            (slot >= 0 && (count == 0 || static_cast<size_t>(slot) >= capture->values.size() / count))) {
        ...
        if ((round_layer || round_deepstack) && tensor->type == GGML_TYPE_F32) {
            const QwenBF16RoundStatus status = qwen_bf16_round_cuda(tensor, count, capture->error);
            if (status == QwenBF16RoundStatus::error) {
                return false;
            }
            rounded_on_device = status == QwenBF16RoundStatus::success;
        }
        if (slot >= 0 && capture->seen.size() > 1 && tensor->type == GGML_TYPE_F32) {
            const QwenBF16RoundStatus status =
                qwen_bf16_capture_cuda(tensor, count, static_cast<size_t>(slot) * count, capture->values.size(),
                                       capture->cuda_capture, capture->error);
            if (status == QwenBF16RoundStatus::error) {
                return false;
            }
        ...
        if (captured_on_device) {
            if (round_layer) {
                ...
                capture->rounded_deepstack_layers[static_cast<size_t>(deepstack_layer)] = 1;
            }
            return true;
        }
        ...
        if (rounded_on_device) {
            if (slot >= 0) {
                ggml_backend_tensor_get(tensor, rounded.data(), 0, count * sizeof(float));
            }
        } else if (tensor->type == GGML_TYPE_F32) {
            std::vector<float> source(count);
            ggml_backend_tensor_get(tensor, source.data(), 0, count * sizeof(float));
            for (size_t index = 0; index < count; ++index) {
                rounded[index] = ggml_bf16_to_fp32(ggml_fp32_to_bf16(source[index]));
            }
        } else if (tensor->type == GGML_TYPE_F16) {
            if (round_layer || round_deepstack) {
                capture->error = "Qwen3-VL BF16 residual-boundary roundtrip requires F32 tensors";
                return false;
            }
            std::vector<ggml_fp16_t> source(count);
            ggml_backend_tensor_get(tensor, source.data(), 0, count * sizeof(ggml_fp16_t));
            for (size_t index = 0; index < count; ++index) {
                rounded[index] = ggml_bf16_to_fp32(ggml_fp32_to_bf16(ggml_fp16_to_fp32(source[index])));
            }
            ...
            ggml_backend_tensor_get(tensor, source.data(), 0, count * sizeof(ggml_bf16_t));
            for (size_t index = 0; index < count; ++index) {
                rounded[index] = ggml_bf16_to_fp32(source[index]);
            }
        } else {
            capture->error =
                std::string("unsupported Qwen3-VL hidden-state output type: ") + ggml_type_name(tensor->type);
            return false;
        }
        if ((round_layer || round_deepstack) && !rounded_on_device) {
            ggml_backend_tensor_set(tensor, rounded.data(), 0, count * sizeof(float));
        }
        if (round_layer || round_deepstack) {
            if (round_layer) {
                capture->rounded_layers[static_cast<size_t>(layer)] = 1;
        ...
        if (slot >= 0) {
            float * destination = capture->values.data() + static_cast<size_t>(slot) * count;
            std::copy(rounded.begin(), rounded.end(), destination);
            capture->seen[static_cast<size_t>(slot)] = 1;
        }
        return true;
    } catch (const std::exception & exception) {
        capture->error = std::string("failed to capture Qwen3-VL hidden-state output: ") + exception.what();
        return false;
    } catch (...) {
...
std::vector<llama_token> tokenize(const llama_vocab * vocab, const std::string & text, bool parse_special) {
    const int32_t required =
        -llama_tokenize(vocab, text.data(), static_cast<int32_t>(text.size()), nullptr, 0, false, parse_special);
    if (required <= 0) {
        return {};
    }
    std::vector<llama_token> tokens(static_cast<size_t>(required));
    const int32_t count = llama_tokenize(vocab, text.data(), static_cast<int32_t>(text.size()), tokens.data(), required,
                                         false, parse_special);
    if (count != required) {
        return {};
    }
...
std::string apply_chat_template(const llama_model * model, QwenVLArchitecture architecture,
    ...
    const size_t message_offset = architecture == QwenVLArchitecture::qwen2_5_vl ? 0U : 1U;
    const size_t message_count  = architecture == QwenVLArchitecture::qwen2_5_vl ? 2U : 1U;
    const int32_t required =
        llama_chat_apply_template(chat_template, messages + message_offset, message_count, true, nullptr, 0);
    if (required < 0) {
    ...
    const int32_t written = llama_chat_apply_template(chat_template, messages + message_offset, message_count, true,
...
bool validate_image(const Qwen3VLImageView & image, const Qwen3VLBridgeConfig & config, PackedImageLayout & layout,
                    std::string & error) {
    (void)config;
    layout = PackedImageLayout{};
    if (image.data == nullptr || image.channels != 3 || image.width <= 0 || image.height <= 0) {
    ...
    if (image.stride_bytes < 0 ||
        (image.stride_bytes > 0 && static_cast<size_t>(image.stride_bytes) < layout.row_bytes)) {
        error = "Qwen3-VL image stride is smaller than a packed RGB row";
        return false;
    }
    layout.stride_bytes = image.stride_bytes > 0 ? static_cast<size_t>(image.stride_bytes) : layout.row_bytes;
    if (height > std::numeric_limits<size_t>::max() / layout.row_bytes ||
        (height > 1U && height - 1U > (std::numeric_limits<size_t>::max() - layout.row_bytes) / layout.stride_bytes)) {
...
std::vector<uint8_t> pack_image(const Qwen3VLImageView & image, const PackedImageLayout & layout) {
    std::vector<uint8_t> packed(layout.packed_bytes);
    for (int row = 0; row < image.height; ++row) {
        std::memcpy(packed.data() + static_cast<size_t>(row) * layout.row_bytes,
                    image.data + static_cast<size_t>(row) * layout.stride_bytes, layout.row_bytes);
    }
    return packed;
}

void tokenize_multimodal_prompt(const Qwen3VLBridgeConfig & config, QwenVLArchitecture architecture,
    ...
    for (const Qwen3VLImageView & image : images) {
        ...
        bitmaps.entries.emplace_back(static_cast<uint32_t>(image.width), static_cast<uint32_t>(image.height),
    ...
    const std::string content   = build_qwen_media_content(images.size(), instruction, mtmd_default_marker());
    const std::string formatted = apply_chat_template(model, architecture, content);
    ...
    const int32_t tokenize_result =
        mtmd_tokenize(vision, chunks.ptr.get(), &input_text, bitmap_ptrs.data(), bitmap_ptrs.size());
...
struct Qwen3VLBridge::Impl {
    ...
    void refresh_backend_name() const {
        const bool accelerator = text_placement.accelerator_compute && vision_placement.accelerator_compute;
        const bool cpu         = text_placement.cpu_compute || vision_placement.cpu_compute;
        if (accelerator && !cpu) {
            backend_name = compiled_backend_name();
        } else if (!text_placement.accelerator_compute && !vision_placement.accelerator_compute &&
...
void copy_token_embedding(const ggml_tensor * token_embeddings, llama_token token, size_t hidden_size,
    ...
    if (token_embeddings == nullptr || destination == nullptr || token < 0 ||
        token_embeddings->ne[0] != static_cast<int64_t>(hidden_size) || token >= token_embeddings->ne[1] ||
        !ggml_is_contiguous(token_embeddings) || token_embeddings->buffer == nullptr) {
        throw std::runtime_error("Qwen3-VL token embedding table is incompatible");
    }
    const size_t row_stride = token_embeddings->nb[1];
    if (row_stride == 0 || static_cast<size_t>(token) > std::numeric_limits<size_t>::max() / row_stride) {
    ...
    case GGML_TYPE_F32:
        ggml_backend_tensor_get(token_embeddings, destination, row_offset, hidden_size * sizeof(float));
        break;
    case GGML_TYPE_F16: {
        std::vector<ggml_fp16_t> row(hidden_size);
        ggml_backend_tensor_get(token_embeddings, row.data(), row_offset, hidden_size * sizeof(ggml_fp16_t));
    ...
    case GGML_TYPE_BF16: {
        ...
        ggml_backend_tensor_get(token_embeddings, row.data(), row_offset, hidden_size * sizeof(ggml_bf16_t));
        for (size_t index = 0; index < hidden_size; ++index) {
...
PreparedMultimodalBatch prepare_multimodal_batch(const Qwen3VLBridgeConfig & config, llama_model * model,
                                                 mtmd_context * vision, const mtmd::input_chunks & chunks) {
    ...
    for (size_t chunk_index = 0; chunk_index < chunks.size(); ++chunk_index) {
        ...
        if (chunk_tokens == 0 || chunk_tokens > std::numeric_limits<size_t>::max() - prepared.token_count) {
    ...
    if (hidden_size == 0 || input_size != static_cast<size_t>(llama_model_n_embd_inp(model)) ||
        input_size < hidden_size || prepared.token_count > std::numeric_limits<size_t>::max() / input_size) {
    ...
    for (size_t chunk_index = 0; chunk_index < chunks.size(); ++chunk_index) {
        ...
        if (chunk_positions <= 0 || position_offset > std::numeric_limits<llama_pos>::max() - chunk_positions) {
        ...
        if (type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
            ...
            const llama_token * tokens = mtmd_input_chunk_get_tokens_text(chunk, &text_token_count);
            ...
            for (size_t local_index = 0; local_index < chunk_tokens; ++local_index) {
                ...
                const llama_pos position         = position_offset + static_cast<llama_pos>(local_index);
            ...
            if (image_tokens == nullptr || mtmd_image_tokens_get_n_tokens(image_tokens) != chunk_tokens) {
            ...
            float * destination              = prepared.embeddings.data() + token_offset * input_size;
            for (size_t element = 0; element < image_element_count; ++element) {
                destination[element] = ggml_bf16_to_fp32(ggml_fp32_to_bf16(image_embeddings[element]));
            }
            for (size_t local_index = 0; local_index < chunk_tokens; ++local_index) {
                const size_t global_index = token_offset + local_index;
                const mtmd_decoder_pos position =
                    mtmd_image_tokens_get_decoder_pos(image_tokens, position_offset, local_index);
                prepared.positions[global_index]                             = static_cast<llama_pos>(position.t);
                prepared.positions[prepared.token_count + global_index]      = static_cast<llama_pos>(position.y);
                prepared.positions[prepared.token_count * 2U + global_index] = static_cast<llama_pos>(position.x);
                prepared.positions[prepared.token_count * 3U + global_index] = static_cast<llama_pos>(position.z);
            }
        } else {
...
void export_prepared_inputs(const Qwen3VLBridgeConfig & config, const llama_vocab * vocab,
                            const mtmd::input_chunks & chunks, const PreparedMultimodalBatch & prepared,
                            std::vector<int64_t> & input_ids, std::vector<uint8_t> & attention_mask,
                            std::vector<int64_t> & image_grid_thw) {
    const std::vector<llama_token> image_pad_tokens = tokenize(vocab, "<|image_pad|>", true);
    if (image_pad_tokens.size() != 1) {
        throw std::runtime_error("Qwen3-VL vocabulary does not expose a unique <|image_pad|> token");
    }
    ...
    for (llama_token token : prepared.token_ids) {
        input_ids.push_back(token < 0 ? static_cast<int64_t>(image_pad_tokens.front()) : static_cast<int64_t>(token));
    }
    ...
    for (size_t chunk_index = 0; chunk_index < chunks.size(); ++chunk_index) {
        ...
        for (size_t token = 0; token < image_token_count; ++token) {
            const mtmd_decoder_pos position = mtmd_image_tokens_get_decoder_pos(image_tokens, 0, token);
            if (position.t != 0 || position.z != 0) {
                throw std::runtime_error("Qwen3-VL image token grid does not use the expected M-RoPE layout");
            }
        ...
        if (merged_width == 0 || merged_height == 0 || merged_width > static_cast<size_t>(INT64_MAX) / merge ||
...
std::unique_ptr<Qwen3VLBridge> Qwen3VLBridge::load(const Qwen3VLBridgeConfig & config, std::string & error) {
    error.clear();
    ...
    if (config.text_path.empty() || config.mmproj_path.empty() || config.bundle_uuid.empty() ||
        config.hidden_size <= 0 || config.input_embedding_size <= 0 || config.vocab_size <= 0 || !action_config_valid ||
        config.expected_image_count <= 0 || config.image_min_tokens <= 0 ||
        config.image_max_tokens < config.image_min_tokens || config.image_spatial_merge_size <= 0 ||
        config.n_ctx <= 0 || config.n_batch <= 0) {
    ...
    impl->config = config;
    try {
        const std::string mmproj_uuid = gguf_string_metadata(config.mmproj_path, "general.source.uuid");
        if (mmproj_uuid != config.bundle_uuid) {
            throw std::runtime_error("Qwen3-VL mmproj bundle UUID does not match the policy");
        }
        const std::string projector_type = gguf_string_metadata(config.mmproj_path, "clip.projector_type");
        ...
        if (!qwen_vl_resolve_architecture(model_metadata(impl->model, "general.architecture"), projector_type,
                                          impl->architecture, profile_error)) {
        ...
        if (config.input_embedding_size % config.hidden_size != 0) {
            throw std::runtime_error("Qwen-VL input embedding width is not an integral hidden-state layout");
        }
        const int deepstack_layer_count = config.input_embedding_size / config.hidden_size - 1;
        if ((impl->architecture == QwenVLArchitecture::qwen2_5_vl && deepstack_layer_count != 0) ||
            (impl->architecture == QwenVLArchitecture::qwen3_vl &&
             (deepstack_layer_count <= 0 || deepstack_layer_count > llama_model_n_layer(impl->model)))) {
            throw std::runtime_error("Qwen-VL input embedding layout does not match the detected architecture");
        }
        impl->deepstack_layer_count = static_cast<size_t>(deepstack_layer_count);
        ...
        if (!config.action_token.empty()) {
            const std::vector<llama_token> action_tokens = tokenize(impl->vocab, config.action_token, true);
            if (action_tokens.size() != 1 || action_tokens.front() != config.action_token_id) {
                throw std::runtime_error("Qwen3-VL action token mapping does not match the policy metadata");
            }
        }
        ...
        context_params.flash_attn_type =
            config.flash_text_attention ? LLAMA_FLASH_ATTN_TYPE_ENABLED : LLAMA_FLASH_ATTN_TYPE_DISABLED;
        impl->layer_capture.placement                      = &impl->text_placement;
        impl->layer_capture.bf16_residual_layer_boundaries = config.bf16_residual_layer_boundaries;
        if (config.bf16_residual_layer_boundaries) {
            impl->layer_capture.expected_deepstack_layer_count = impl->deepstack_layer_count;
        }
        ...
        if (llama_n_batch(impl->context) != static_cast<uint32_t>(config.n_batch) ||
            ...
            throw std::runtime_error("Qwen3-VL text context did not preserve the requested batch/ubatch contract");
        }
        ...
        if (config.verbosity >= 1) {
            std::fprintf(stderr,
                         ...
                         __func__, qwen_vl_architecture_name(impl->architecture), impl->backend_name.c_str(),
                         llama_model_n_embd_out(impl->model), llama_model_n_embd_inp(impl->model),
                         impl->deepstack_layer_count, config.expected_image_count, config.image_min_tokens,
                         config.image_max_tokens, llama_n_ctx(impl->context), llama_n_batch(impl->context),
                         llama_n_ubatch(impl->context), config.disable_text_backend_native_graphs ? "true" : "false",
...
bool Qwen3VLBridge::extract_token_embeddings(const std::vector<Qwen3VLImageView> & images,
                                             const std::string & instruction, int32_t token_id, size_t token_count,
                                             std::vector<float> & embeddings, std::string & error) {
    embeddings.clear();
    error.clear();
    if (impl_ == nullptr || impl_->model == nullptr || impl_->context == nullptr || impl_->vision == nullptr ||
        impl_->vocab == nullptr) {
    ...
    try {
        mtmd::input_chunks chunks;
        tokenize_multimodal_prompt(impl_->config, impl_->architecture, impl_->model, impl_->vision, images, instruction,
                                   chunks);
        PreparedMultimodalBatch prepared = prepare_multimodal_batch(impl_->config, impl_->model, impl_->vision, chunks);
        ...
        if (capture.seen.size() != 1 || capture.seen.front() == 0) {
            throw std::runtime_error("Qwen-VL did not expose the final conditioning output");
        }

        embeddings.resize(token_count * capture.hidden_size);
        for (size_t output_index = 0; output_index < matches.size(); ++output_index) {
            const float * hidden = capture.values.data() + matches[output_index] * capture.hidden_size;
            std::copy_n(hidden, capture.hidden_size, embeddings.data() + output_index * capture.hidden_size);
        }
...
bool Qwen3VLBridge::extract_full_hidden_states(const std::vector<Qwen3VLImageView> & images,
                                               const std::string & instruction, std::vector<float> & hidden_states,
...
bool Qwen3VLBridge::extract_layer_hidden_states(const std::vector<Qwen3VLImageView> & images,
    ...
    if (impl_->deepstack_layer_count > static_cast<size_t>(std::numeric_limits<int>::max())) {
        error = "Qwen-VL DeepStack layer count exceeds the supported range";
        return false;
    }
    const int deepstack_layer_count = static_cast<int>(impl_->deepstack_layer_count);
    int result_norm_slot            = -1;
    for (size_t slot = 0; slot < hidden_tuple_indices.size(); ++slot) {
        ...
        if (!qwen_vl_hidden_state_source(impl_->architecture, model_layer_count, deepstack_layer_count, tuple_index,
                                         source, error)) {
            return false;
        }
    ...
    try {
        ...
        if (prepared.token_count > std::numeric_limits<size_t>::max() / hidden_size ||
            prepared.token_count * hidden_size > std::numeric_limits<size_t>::max() / requested_layers) {
        ...
        begin_layer_boundary_tracking(capture, static_cast<size_t>(model_layer_count));
        capture.error.clear();
        capture.enabled = true;
        std::fill(prepared.outputs.begin(), prepared.outputs.end(), int8_t{1});
        ...
        if (std::any_of(capture.seen.begin(), capture.seen.end(), [](uint8_t seen) { return seen == 0; })) {
            throw std::runtime_error("Qwen3-VL did not expose every requested hidden-state output");
        }
...
bool Qwen3VLBridge::generate_autoregressive(const std::vector<Qwen3VLImageView> & images,
                                            const std::string & instruction, const QwenVLGenerationConfig & generation,
    ...
    if (generation.max_length == 0 || generation.max_length > static_cast<size_t>(llama_n_ctx(impl_->context)) ||
        generation.max_length > static_cast<size_t>(INT32_MAX) || generation.top_k != 1 ||
        generation.eos_token_ids.empty() || !std::isfinite(generation.repetition_penalty) ||
        generation.repetition_penalty <= 0.0f) {
        error = "Qwen-VL autoregressive generation configuration is incompatible";
        return false;
    }
    std::vector<uint8_t> eos_seen(static_cast<size_t>(impl_->config.vocab_size), uint8_t{0});
    for (int32_t eos : generation.eos_token_ids) {
        if (eos < 0 || eos >= impl_->config.vocab_size || eos_seen[static_cast<size_t>(eos)] != 0) {
    ...
    try {
        ...
        PreparedMultimodalBatch prepared = prepare_multimodal_batch(impl_->config, impl_->model, impl_->vision, chunks);
        if (prepared.token_count > llama_n_batch(impl_->context)) {
            throw std::runtime_error("Qwen-VL multimodal prompt exceeds n_batch; increase --n-batch");
        }
        if (prepared.token_count > generation.max_length ||
            prepared.token_count > static_cast<size_t>(llama_n_ctx(impl_->context)) ||
            prepared.position_count > static_cast<llama_pos>(llama_n_ctx(impl_->context))) {
            throw std::runtime_error("Qwen-VL multimodal prompt exceeds the FAST max_length/n_ctx contract");
        }

        std::vector<int64_t> input_ids;
        std::vector<uint8_t> attention_mask;
        std::vector<int64_t> image_grid_thw;
        export_prepared_inputs(impl_->config, impl_->vocab, chunks, prepared, input_ids, attention_mask,
                               image_grid_thw);
        if (input_ids.size() != prepared.token_count) {
            throw std::runtime_error("Qwen-VL multimodal prompt token export is inconsistent");
        }
        ...
        for (int64_t input_id : input_ids) {
            if (input_id < 0 || input_id >= impl_->config.vocab_size) {
                throw std::runtime_error("Qwen-VL multimodal prompt contains an out-of-vocabulary token");
            }
        ...
        if (decode_and_synchronize(impl_->context, prompt_batch) != 0) {
            throw std::runtime_error("failed to evaluate the Qwen-VL autoregressive prompt");
        }
        ...
        while (result.full_sequence.size() < generation.max_length) {
            ...
            if (!qwen_vl_select_repetition_penalized_top1(logits, static_cast<size_t>(impl_->config.vocab_size),
            ...
            if (eos_seen[static_cast<size_t>(next)] != 0 || result.full_sequence.size() == generation.max_length) {
                break;
            }

            const size_t generation_index = result.continuation.size() - 1U;
            if (generation_index >
                static_cast<size_t>(std::numeric_limits<llama_pos>::max() - prepared.position_count)) {
                throw std::runtime_error("Qwen-VL autoregressive M-RoPE position overflow");
            }
            llama_token token           = static_cast<llama_token>(next);
            llama_pos position          = prepared.position_count + static_cast<llama_pos>(generation_index);
            int32_t sequence_count      = 1;
            llama_seq_id sequence_value = 0;
            llama_seq_id * sequence     = &sequence_value;
            int8_t output               = 1;
            llama_batch token_batch{
                1, &token, nullptr, &position, &sequence_count, &sequence, &output,
            };
            if (decode_and_synchronize(impl_->context, token_batch) != 0) {
                throw std::runtime_error("failed to evaluate an incremental Qwen-VL generation token");
            }
        }
