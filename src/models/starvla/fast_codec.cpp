...
bool decode_utf8_strict(const std::string & input, std::vector<uint32_t> & output, std::string & error) {
    output.clear();
...
bool compile_token_bytes(const std::vector<std::string> & vocab_by_id, std::vector<std::vector<uint8_t>> & token_bytes,
                         std::string & error) {
...
std::unique_ptr<FastCodec> FastCodec::create(FastCodecConfig config, std::vector<std::string> vocab_by_id,
    ...
    return std::unique_ptr<FastCodec>(new FastCodec(config, std::move(token_bytes), std::move(fast_to_vlm_id)));
}

std::unique_ptr<FastCodec> FastCodec::create_compiled(FastCodecConfig config, std::vector<int32_t> token_offsets,
                                                      std::vector<uint8_t> token_bytes,
                                                      std::vector<int32_t> fast_to_vlm_id, std::string & error) {
    error.clear();
    if (!std::isfinite(config.scale) || config.scale == 0.0 || config.vocab_size == 0 || config.time_horizon == 0 ||
        config.action_dim == 0 || config.vocab_size > static_cast<size_t>(std::numeric_limits<int32_t>::max())) {
        error = "StarVLA FAST compiled codec dimensions and scale are invalid";
        return nullptr;
    }
    if (config.vocab_size == std::numeric_limits<size_t>::max() || token_offsets.size() != config.vocab_size + 1U ||
        fast_to_vlm_id.size() != config.vocab_size || token_offsets.empty() || token_offsets.front() != 0 ||
        token_offsets.back() < 0 || static_cast<size_t>(token_offsets.back()) != token_bytes.size()) {
    ...
    for (size_t index = 0; index < config.vocab_size; ++index) {
        ...
        if (begin < 0 || end <= begin || static_cast<size_t>(end) > token_bytes.size()) {
    ...
    return std::unique_ptr<FastCodec>(new FastCodec(config, std::move(pieces), std::move(fast_to_vlm_id)));
}
...
bool FastCodec::map_fast_to_vlm(const std::vector<int32_t> & fast_ids, std::vector<int32_t> & vlm_ids,
...
bool FastCodec::map_vlm_to_fast(const std::vector<int32_t> & vlm_ids, std::vector<int32_t> & fast_ids,
                                std::string & error) const {
    fast_ids.clear();
    error.clear();
    ...
    for (int32_t vlm_id : vlm_ids) {
        const auto found = std::lower_bound(
            ...
            [](const std::pair<int32_t, int32_t> & entry, int32_t value) { return entry.first < value; });
...
bool FastCodec::extract_fast_tokens(const std::vector<int32_t> & generated_ids, std::vector<int32_t> & fast_ids,
                                    std::string & error) const {
    fast_ids.clear();
    error.clear();
...
bool FastCodec::byte_level_decode(const std::vector<int32_t> & fast_ids, std::vector<uint32_t> & codepoints,
                                  std::string & error) const {
    codepoints.clear();
    error.clear();
    if (fast_ids.size() > kMaximumTokenSequence) {
...
bool FastCodec::decode_fast_tokens(const std::vector<std::vector<int32_t>> & batch_fast_ids, FastDecodeResult & result,
                                   std::string & error) const {
    result = {};
    error.clear();
    ...
    for (size_t batch = 0; batch < batch_fast_ids.size(); ++batch) {
        ...
        for (size_t action = 0; action < config_.action_dim; ++action) {
            const double dc = (static_cast<double>(codepoints[action]) + config_.min_token) / config_.scale;
            for (size_t time = 0; time < config_.time_horizon; ++time) {
                ...
                for (size_t frequency = 1; frequency < config_.time_horizon; ++frequency) {
                    ...
                    const double coefficient =
                        (static_cast<double>(codepoints[coefficient_index]) + config_.min_token) / config_.scale;
                    const double angle = kPi * static_cast<double>(frequency) * static_cast<double>(2U * time + 1U) /
...
bool FastCodec::decode_vlm_action_tokens(const std::vector<std::vector<int32_t>> & batch_vlm_ids,
...
bool FastCodec::decode_generated_tokens(const std::vector<std::vector<int32_t>> & batch_generated_ids,
                                        FastDecodeResult & result, std::string & error) const {
