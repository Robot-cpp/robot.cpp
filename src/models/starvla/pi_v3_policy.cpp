  ...
  protected:
    bool parse_metadata(gguf_context * gguf) override {
        if (require_string(gguf, "general.architecture") != "starvla-policy" ||
            require_i32(gguf, "starvla.schema_version") != 1 || require_string(gguf, "starvla.framework") != "pi_v3") {
        ...
        if (config_.bundle_uuid.empty() || config_.text_filename.empty() || config_.mmproj_filename.empty()) {
        ...
        config_.attention_head_count     = require_i32(gguf, "starvla.pi_v3.attention_head_count");
        config_.attention_head_dim       = require_i32(gguf, "starvla.pi_v3.attention_head_dim");
        config_.feed_forward_dim         = require_i32(gguf, "starvla.pi_v3.feed_forward_dim");
        config_.mlp_hidden_dim           = require_i32(gguf, "starvla.pi_v3.mlp_hidden_dimension");
        config_.future_token_count       = require_i32(gguf, "starvla.pi_v3.future_token_count");
        config_.action_position_count    = require_i32(gguf, "starvla.pi_v3.action_position_count");
        config_.no_state_sequence_length = require_i32(gguf, "starvla.pi_v3.no_state_sequence_length");
        config_.timestep_projection_dim  = require_i32(gguf, "starvla.pi_v3.timestep_projection_dim");
        config_.num_timestep_buckets     = require_i32(gguf, "starvla.pi_v3.num_timestep_buckets");
        config_.num_inference_timesteps  = require_i32(gguf, "starvla.pi_v3.num_inference_timesteps");
        config_.ada_norm_epsilon         = require_f32(gguf, "starvla.pi_v3.ada_norm_epsilon");
        config_.projector_norm_epsilon   = require_f32(gguf, "starvla.pi_v3.projector_norm_epsilon");
        ...
        const bool valid =
            ...
            config_.image_max_token_count >= config_.image_min_token_count && config_.dit_width > 0 &&
            config_.block_count == kReleasedLayerCount && config_.projector_count == config_.block_count &&
            ...
            config_.feed_forward_dim > 0 && config_.mlp_hidden_dim > 0 && config_.action_dim > 0 &&
            config_.horizon > 0 && config_.future_token_count > 0 && config_.action_position_count >= config_.horizon &&
            ...
            config_.ada_norm_epsilon > 0.0f && config_.projector_norm_epsilon > 0.0f && config_.euler_dt > 0.0f;
        if (!valid) {
    ...
    bool bind_tensors(ggml_context * ctx_data) override {
        ...
        for (int projector = 0; projector < config_.projector_count; ++projector) {
            const std::string prefix = "starvla.policy.pi_v3.projector." + std::to_string(projector) + ".";
        ...
        for (const PIV3BlockWeights & current : weights_.blocks) {
            if (!has_shape(current.ada_norm_weight, {width, 2 * width}) ||
                ...
                !has_shape(current.query_bias, {width}) || !has_shape(current.key_weight, {width, width}) ||
                !has_shape(current.key_bias, {width}) || !has_shape(current.value_weight, {width, width}) ||
...
struct PIV3Policy::Impl {
    ...
    void build_graph(size_t token_count) {
        ...
        auto bf16_roundtrip = [&](ggml_tensor * tensor) {
            return ggml_cast(graph_context, ggml_cast(graph_context, tensor, GGML_TYPE_BF16), GGML_TYPE_F32);
        };
        ...
        auto projector_linear = [&](ggml_tensor * value, ggml_tensor * weight, ggml_tensor * bias) {
            ...
            ggml_tensor * projected   = ggml_mul_mat(graph_context, bf16_weight, bf16_value);
            ggml_mul_mat_set_prec(projected, GGML_PREC_F32);
            projected = ggml_add(graph_context, projected, bf16_roundtrip(bias));
            return bf16_roundtrip(projected);
        };
        auto ada_norm = [&](ggml_tensor * value, ggml_tensor * temb, const PIV3BlockWeights & block) {
        ...
        for (int layer = 0; layer < config.projector_count; ++layer) {
            ...
            ggml_tensor * layer_hidden =
                ggml_view_2d(graph_context, hidden_input, config.qwen_hidden_dim, static_cast<int64_t>(token_count),
                             hidden_input->nb[1], static_cast<size_t>(layer) * hidden_input->nb[2]);
            ...
            ggml_tensor * projected =
                projector_linear(layer_hidden, projector.projection_weight, projector.projection_bias);
        ...
        for (int step = 0; step < 4; ++step) {
            ...
            for (int block_index = 0; block_index < config.block_count; ++block_index) {
                ...
                ggml_tensor * attended =
                    attention(normalized, projected_hidden_states[static_cast<size_t>(block_index)], block);
...
std::unique_ptr<PIV3Policy> PIV3Policy::load(const std::string & path, int n_threads, int verbosity,
    ...
    try {
        ...
        if (verbosity >= 1) {
            std::fprintf(stderr, "%s: backend=%s qwen=%d width=%d layers=%d horizon=%d action_dim=%d profiles=%zu\n",
...
bool PIV3Policy::evaluate(const float * qwen_hidden_states, size_t hidden_element_count,
    ...
    return evaluate_internal(qwen_hidden_states, hidden_element_count, qwen_attention_mask, mask_element_count,
                             initial_noise, noise_element_count, normalized_actions, error);
}

bool PIV3Policy::evaluate_internal(const float * qwen_hidden_states, size_t hidden_element_count,
...
bool PIV3Policy::unnormalize(const std::vector<float> & normalized_actions, const std::string & profile_key_value,
