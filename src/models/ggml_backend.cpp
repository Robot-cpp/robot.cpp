...
static bool mode_for_device(ggml_backend_dev_t dev, backend_mode & mode, std::string & error) {
    ...
    error = std::string("unknown GGML backend registry: ") + (reg_name && reg_name[0] ? reg_name : "(null)");
    return false;
}
...
bool backend_loader::load(ggml_backend_t & cpu_backend, std::vector<ggml_backend_t> & backends,
...
bool backend_loader::init_backends(ggml_backend_t & cpu_backend, std::vector<ggml_backend_t> & backends, bool use_accel,
    ...
    if (verbosity >= 1) {
        std::fprintf(stderr, "%s: desired_mode=%s resolved_mode=%s backend_count=%zu\n", __func__,
                     backend_mode_name(wanted), backend_mode_name(mode_), backends.size());
    }

    return true;
}

bool backend_loader::init_buft_policy(ggml_backend_t cpu_backend, const std::vector<ggml_backend_t> & backends,
                                      backend_buft_policy & buft_policy, int verbosity) {
    if (!cpu_backend) {
    ...
    if (verbosity >= 1) {
        std::fprintf(stderr, "%s: model_buft=%s runtime_buft=%s host_buft=%s\n", __func__,
                     backend_buft_name(buft_policy.model_buft), backend_buft_name(buft_policy.runtime_buft),
...
bool backend_loader::init_scheduler(std::vector<ggml_backend_t> & backends, ggml_backend_sched_t & scheduler,
    ...
    scheduler = ggml_backend_sched_new(backends.data(), sched_bufts.data(), backends.size(), scheduler_config.max_nodes,
                                       scheduler_config.parallel, scheduler_config.op_offload);
    if (!scheduler) {
        return fail("failed to initialize backend scheduler");
    }

    if (verbosity >= 1) {
        std::fprintf(stderr, "%s: enabled scheduler with %zu backend(s)\n", __func__, backends.size());
    }
