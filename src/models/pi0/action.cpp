...
void sample_flow_euler(int steps, std::mt19937 & rng, int horizon, int action_dim,
                       const std::vector<float> * initial_noise, VelocityFn velocity, std::vector<float> & out) {
...
void pi0_state_context(const Pi0Context & ctx, const std::vector<float> & state, std::vector<float> & out) {
    ...
    pi0_linear_batch(ctx, ctx.components.state.runtime, state_w, state_b, state, 1, out,
...
class Pi0ActionVelocityGraph {
  public:
    Pi0ActionVelocityGraph(const Pi0Context & ctx, const std::vector<float> & state_context)
        : ctx_(ctx), state_context_(state_context), runtime_(ctx.components.action_decoder.runtime),
  ...
  private:
    void init_shape() {
        ...
        if (ctx_.weights.action_in_w == nullptr || pi0.action.expert_layers <= 0 || pi0.action.expert_width <= 0 ||
            pi0.action.expert_q_out <= 0 || pi0.action.expert_kv_out <= 0 || ctx_.weights.action_layers.empty() ||
            ctx_.weights.action_layers[0].q == nullptr || ctx_.weights.action_layers[0].k == nullptr ||
        ...
        if (suffix_count_ <= 0 || batch_ <= 0 || action_dim_ <= 0 || layers_ <= 0 || width_ <= 0 || q_out_ <= 0 ||
            kv_out_ <= 0 || mlp_width_ <= 0 || heads_ <= 0 || kv_heads_ <= 0 || head_dim_ <= 0 ||
        ...
        if (has_state_context_) {
            ...
            ggml_fp32_to_fp16_row(attention_mask_.data(), attention_mask_f16_.data(),
    ...
    void build_graph() {
        ...
        if (in_w == nullptr || in_b == nullptr || time_in_w == nullptr || time_in_b == nullptr ||
            time_out_w == nullptr || time_out_b == nullptr || out_w == nullptr || out_b == nullptr) {
        ...
        ggml_tensor * action_tokens = ggml_add(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, in_w, 2), action_input_),
                                               pi0_f32_weight(gctx_, ctx_, in_b, 1));
        ggml_tensor * action_time   = ggml_concat(gctx_, action_tokens, time_input_, 0);
        ggml_tensor * time_hidden =
            ggml_silu(gctx_, ggml_add(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, time_in_w, 2), action_time),
                                      pi0_f32_weight(gctx_, ctx_, time_in_b, 1)));
        ggml_tensor * hidden = ggml_add(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, time_out_w, 2), time_hidden),
        ...
        for (int layer = 0; layer < layers_; ++layer) {
            ...
            if (q_w == nullptr || k_w == nullptr || v_w == nullptr || attn_out_w == nullptr || gate_w == nullptr ||
            ...
            ggml_tensor * q =
                ggml_cont_2d(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, q_w, 2), normed), q_out_, suffix_count_);
            ggml_tensor * k =
                ggml_cont_2d(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, k_w, 2), normed), kv_out_, suffix_count_);
            ggml_tensor * v =
                ggml_cont_2d(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, v_w, 2), normed), kv_out_, suffix_count_);
            ...
            ggml_tensor * q_rot = ggml_rope_ext(gctx_, q, pos_, nullptr, head_dim_, GGML_ROPE_TYPE_NEOX, 8192, 10000.0f,
                                                1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
            ggml_tensor * k_rot = ggml_rope_ext(gctx_, k, pos_, nullptr, head_dim_, GGML_ROPE_TYPE_NEOX, 8192, 10000.0f,
                                                1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
            ...
            if (prefix_tokens_ > 0) {
                ...
                if (prefix_k == nullptr || prefix_v == nullptr || prefix_k->ne[0] != head_dim_ ||
                    prefix_k->ne[1] != kv_heads_ || prefix_k->ne[2] != static_cast<int64_t>(prefix_tokens_) ||
                    prefix_v->ne[0] != head_dim_ || prefix_v->ne[1] != kv_heads_ ||
        ...
        y_                          = ggml_add(gctx_, ggml_mul_mat(gctx_, pi0_weight(ctx_, out_w, 2), output_tokens),
        ...
        if (kq_mask_ != nullptr) {
            ggml_backend_tensor_set(kq_mask_, attention_mask_f16_.data(), 0,
    ...
    void fill_time_batch(float time) {
        ...
        for (int row = 0; row < batch_; ++row) {
            std::copy(time_embedding_.begin(), time_embedding_.end(),
                      time_batch_.begin() +
...
void pi0_sample_actions(const Pi0Context & ctx, Pi0RuntimeConfig & runtime, const std::vector<float> & state_context,
                        const std::vector<float> & initial_noise, std::vector<float> & out_actions) {
    Pi0ActionVelocityGraph velocity_graph(ctx, state_context);
    sample_flow_euler(
        runtime.flow_steps, runtime.rng, ctx.config.common.action_horizon, ctx.config.common.action_dim,
        initial_noise.empty() ? nullptr : &initial_noise,
        [&](float time, const std::vector<float> & x, std::vector<float> & v) { velocity_graph.eval(time, x, v); },
        out_actions);
