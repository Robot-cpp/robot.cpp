...
struct smolvla_vision_ctx {
    ...
    struct layer_t {
        ...
        ggml_tensor * ln2_b;
        ggml_tensor * ffn1_w;
        ggml_tensor * ffn1_b; // ffn_down
        ggml_tensor * ffn2_w;
        ggml_tensor * ffn2_b; // ffn_up
    };
...
std::vector<float> smolvla_vision_encode_file(smolvla_vision_ctx * ctx, const char * image_path, int n_threads = 4);
...
std::vector<float> smolvla_vision_encode_raw(smolvla_vision_ctx * ctx, const uint8_t * data, int width, int height,
                                             int channels, int stride_bytes, int n_threads = 4);
