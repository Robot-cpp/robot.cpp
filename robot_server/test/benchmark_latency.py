...
def append_tsv(path: str | None, rows: list[dict[str, float]]) -> None:
    ...
    with output.open("a", encoding="utf-8") as fout:
        ...
        for row in rows:
            fout.write("\t".join(f"{row[name]:.6f}" for name in columns) + "\n")
...
def main() -> int:
    ...
    for i in range(total_iters):
        ...
        rows.append({
            ...
            "vlm_ms": response.timings["vlm_ms"],
    ...
    print(
        "timing_ms "
        f"roundtrip={last['roundtrip_ms']:.2f} "
        f"model_total={last['model_total_ms']:.2f} "
        f"communication_overhead={last['communication_overhead_ms']:.2f}"
    )
    return 0
