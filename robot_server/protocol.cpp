...
static bool validate_f32_array(const std::vector<float> & values, const char * label, std::string & error) {
...
bool decode_predict_request(const std::vector<uint8_t> & payload, predict_request & req, std::string & error) {
    ...
    const uint64_t scalar_bytes = (static_cast<uint64_t>(state_dim) + static_cast<uint64_t>(noise_dim)) * sizeof(float);
