...
echo "[lerobot-so101-client] robot_port=${ROBOT_PORT} camera_key=${CAMERA_KEY} fps=${FPS}"
echo "[lerobot-so101-client] Start smolvla-server first: bash robot_server/shell/launch_robot_server_mac_cpu.sh"

conda run -n "${CONDA_ENV}" lerobot-so101-client \
  --server "${SERVER}" \
