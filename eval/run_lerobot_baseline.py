...
def prepare_libero_config(args: argparse.Namespace, env: dict[str, str]) -> None:
    ...
    if args.conda_env:
        script = (
            ...
            "from pathlib import Path; "
            "from eval.common import ensure_libero_config; "
            "import os; "
...
def main() -> int:
    ...
    env = os.environ.copy()
    env.setdefault("MUJOCO_GL", args.mujoco_gl)
    ...
    if args.numba_cache_dir:
        ...
        env["NUMBA_CACHE_DIR"] = str(args.numba_cache_dir)
    if args.torchinductor_cache_dir:
        args.torchinductor_cache_dir.mkdir(parents=True, exist_ok=True)
        env["TORCHINDUCTOR_CACHE_DIR"] = str(args.torchinductor_cache_dir)
