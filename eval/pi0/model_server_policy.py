...
from eval.common import add_robot_server_to_path
...
class ModelServerPolicy:
    ...
    def predict_action_chunk(self, observation: dict[str, Any], task: str) -> np.ndarray:
        request = {
            "images": [
                {
                    "name": image_name,
        ...
        return np.asarray(response.actions, dtype=np.float32)[:, : self.env_action_dim]
