...
## First-action compare

Use LeRobot as the reference implementation for the first reset observation:

```sh
conda run -n llava python -m eval.pi0.compare_libero_first_action \
  --launch-server \
  --server-bin build-cuda/bin/model-server \
  --suite libero_object \
  --task-ids 0 \
...
The output JSON stores both 50-step chunks, `server_minus_lerobot` error
statistics, LeRobot direct-vs-queue consistency, and request input checks for
state and image tensors.
